export function updateProgressBar(prefix = "", customConfig = null, customState = null) {
    ...
    let total = 0;
    Object.values(config).forEach(w => { if (w.days) total += w.days.length; });
    let done = 0;
    ...
    const dcnt = document.getElementById(prefix + "dcnt");
...
export function renderStructure(config, editMode, onWeek, isPrev = false, prefix = "") {
    ...
    months.forEach((m, idx) => {
        ...
        pan.className = `mpanel ${idx === 0 ? 'on' : ''}`;
        pan.id = `${prefix}mp${m}`;
        pan.innerHTML = `
            <div class="mheader">
                <h2>Month ${m}</h2>
                <p class="${isPrev ? '' : 'editable-global'}" id="${prefix}m-desc-${m}" contenteditable="${editMode && !isPrev}">Study Plan</p>
                <div style="display: ${isPrev ? 'none' : 'flex'}; gap: 10px;">
        ...
        weeks.forEach((wk, wIdx) => {
            ...
            wb.className = `wbtn ${wIdx === 0 ? 'on' : ''}`;
            wb.textContent = config[wk].label;
            const wp = document.createElement('div');
            wp.className = `wpanel ${wIdx === 0 ? 'on' : ''}`;
            wp.id = `${prefix}wp${m}-${wk.split('-')[1]}`;
            wb.onclick = () => {
                pan.querySelectorAll('.wbtn, .wpanel').forEach(e => e.classList.remove('on'));
                wb.classList.add('on'); wp.classList.add('on');
                onWeek(m, wk.split('-')[1], isPrev, prefix);
            };
        ...
        btn.onclick = () => {
            nav.querySelectorAll('.mbtn').forEach(e => e.classList.remove('on'));
            panels.querySelectorAll('.mpanel').forEach(e => e.classList.remove('on'));
            btn.classList.add('on'); pan.classList.add('on');
            pan.querySelector('.wbtn')?.click();
        };
    });
}

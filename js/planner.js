...
export function toggleEditMode(uid) {
    if (!isEditMode) {
        ...
        if (currentConfigStr !== initialConfigStr || currentContentStr !== initialContentStr) {
            addHistoryEntry("Manual Edit Session", sessionInitialConfig, sessionInitialContent);
            saveUserData(uid);
        }
        isEditMode = false;
...
export function renderDynamicOverviewBlocks(uid, prefix = "", customContent = null) {
    ...
    defaults.forEach(def => {
        ...
        card.innerHTML = `
            ...
            <div class="ov-label ${prefix ? '' : 'editable-global'}" id="${prefix}${def.id}-label" contenteditable="${isEditMode && !prefix}">${currentLabel}</div>
            <div class="ov-body ${prefix ? '' : 'editable-global'}" id="${prefix}${def.id}-body" contenteditable="${isEditMode && !prefix}">${currentBody}</div>
        `;
        ...
        grid.querySelectorAll('.del-ov-btn').forEach(btn => {
            btn.onclick = (e) => {
                ...
                if(confirm("Delete this block?")) {
                    pushToUndo();
                    const target = document.getElementById(`container-${bId}`);
...
document.addEventListener('click', (e) => {
    if (e.target && e.target.id === 'addTemplateRowBtn') {
        ...
        if (uid) renderDailyTemplate(uid);
    }
});
